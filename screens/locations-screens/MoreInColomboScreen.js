...
import ActivityCateCardColombo from "../../components/leisure/ActivityCateCardColombo";
import * as Location from "expo-location";
import getPlaces from "../../service/leisure/LeisureService";

import NetInfo from "@react-native-community/netinfo";

const MoreInColomboScreen = () => {
  ...
  const getLocation = async () => {
    ...
    try {
      ...
      // get all places
      let placeData = await getPlaces(locationString);
            ...
            {isSuccess && (
              ...
              >
                {places.map((place, index) => (
                  <ActivityCateCardColombo
                    key={index}
                    ACTIVITY_NAME={place.name}
                    IMG_REF={place.photo_reference}
                    placeItem={place}
                  />
                ))}
              </ScrollView>
...
export default MoreInColomboScreen;
